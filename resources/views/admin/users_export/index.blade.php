...
@section('content')
    <h2 class="text-center">Пользователи выгрузки</h2>
    <p><a href="{{ route('admin.users_export.create') }}" class="btn btn-success"><i class="material-icons">add_circle</i> Добавить</a></p>
            ...
            <tbody>
                @foreach($items as $item)
                    <tr>
                        ...
                        <td>{{ $item->name }}</td>
                        <td>{{ $item->email }}</td>
                        <td><a href="{{ route('admin.users_export.edit', $item->id) }}" class="btn btn-primary btn-small"><i class="material-icons">edit</i></a></td>
